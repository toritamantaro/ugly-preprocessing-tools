...
class BlingfireSplit(TextSplitterBase):
    """
    文分割（sentence segmentation）を行う既存モジュールの動作確認のために作成したラッパー
    make_pipeline()で処理を連結するために作成
    単独で処理する場合は、このクラスを使用する必要はありません。
    ...
    def split_handling(
        ...
        res = text_to_sentences(text).split('\n')
...
if __name__ == "__main__":
    '''
    > python -m cleaner.splitter_blingfire
    '''
    ...
    # texts = texts * 1_000  # 0.0368sec
    # texts = texts * 3

    splitter = BlingfireSplit()
